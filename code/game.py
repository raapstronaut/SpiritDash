...
from settings import (
    ...
    charDescription, gameStartBtn, yellow, hpSprites, score_font, gameover_font, character_speed
)
...
class Game:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if (menu_mouse_pos[0] in range(btn_play_rect.left, btn_play_rect.right) and
                        ...
                        print(f"new state: {self.gamestate}")
                        self.sfxbutton.play()
                        return
    ...
    def choose_character(self):
        ...
        while self.gamestate == "choose character":
            ...
            btn_gotoku = pygame.transform.scale(btn_gotoku, (150, 150))  # Ubah ukuran sesuai kebutuhan
            btn_gotoku_rect = btn_gotoku.get_rect(center=(300, windowH / 2 - 100))
            btn_onre = pygame.image.load("assets/actor/onre/onre_normal.png")
            btn_onre = pygame.transform.scale(btn_onre, (150, 150))  # Ubah ukuran sesuai kebutuhan
            ...
            btn_yurei = pygame.transform.scale(btn_yurei, (150, 150))  # Ubah ukuran sesuai kebutuhan
            btn_yurei_rect = btn_yurei.get_rect(center=(1000, windowH / 2 - 100))


            #back button
            btn_back = pygame.image.load(menuSprites[5])
            btn_back = pygame.transform.scale(btn_back, (100, 35))
            btn_back_rect = btn_back.get_rect(center=(60, 30))
            ...
            if (select_mouse_pos[0] in range(btn_back_rect.left, btn_back_rect.right) and
                select_mouse_pos[1] in range(btn_back_rect.top, btn_back_rect.bottom)):
                btn_back_hover = pygame.image.load(menuSprites[6])
                btn_back_hover = pygame.transform.scale(btn_back_hover, (100, 35))
                screen.blit(btn_back_hover, btn_back_rect)
            ...
            #char descripsi dan start button ketika dipilih
    ...
    def setup_game_assets(self):
        ...
        first_obstacle = get_random_obstacle(windowW + 600, self.charselected)  # Berikan jarak awal yang lebih jauh
        second_obstacle = get_random_obstacle(windowW + 750, self.charselected)  # Jarak 150px dari obstacle pertama
    ...
    def play_game(self):
        ...
        while self.gamestate == "play game":
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == K_SPACE or event.key == K_UP:
                        ...
                        return #Exit method immediately after changing state
                        ...
                        #Quick return to character selection
                        mixer.music.stop()
                        self.reset_all_characters()
                        self.gamestate = "choose character"
                        return #Exit method immediately after changing state
            ...
            #Update HP image based on current health
            ...
            if is_off_screen(self.obstacle_objects.sprites()[0]):
                #remove pipes from tracking before removing them from group
                ...
                new_obs = get_random_obstacle(last_obstacle_x + 300, self.charselected)  # Kurangi dari 1.2 menjadi nilai fixed
            ...
            show_score(str(self.charselected.score), score_font, yellow, int(windowW / 2 + 20) - 30, 20)
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if btn_continue_rect.collidepoint(pause_mouse_pos):
                        mixer.music.unpause()
                        self.isfrompause = True  #continue button
                        self.gamestate = "play game"
