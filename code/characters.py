...
class Character(pygame.sprite.Sprite, ABC):
    ...
    def __init__(self):
        ...
        self.passed_obs = set() #melacak obstacle yang sudah dilewati
    ...
    def get_score(self, obstacle_object, ghost_object):
        ...
        for obs in obstacle_object:
            # hanya pertimbangkan obstacle bawah
            if obs.rect.y > 100: # diasumsikan obstacle atas memiliki y < 100
                ...
                if (ghost_object.sprites()[0].rect.centerx > obs.rect.right and
                    ...
                    break # hanya menghitung satu obstacle per frame
...
class char1(Character):
    ...
    def __init__(self):
        ...
        self.__idObject = "001"
        self.skill = False
        self.__healing_done = False # untuk memastikan skill hanya aktif sekali
        self.images = [
    ...
    # Cek kondisi untuk aktivasi skill
        ...
        # Cek posisi karakter terhadap obstacle
            ...
            and ghost_object.sprites()[0].rect.right < obstacle_objects.sprites()[0].rect.right):
                self.__hp += 1  # Menambah 1 HP
                self.__healing_done = True
                self.skill = True  # Menandakan skill aktif
                self.sound = mixer.Sound(skillactive[0])
                self.sound.play()
                print(f"Gotoku Healing, HP now : {self.__hp}")
    
    # Reset healing_done ketika score bukan kelipatan 10
...
class char2(Character):
    ...
    def __init__(self):
        ...
        self.__idObject = "002"
        self.skill = False
        self.__ghost_mode = False #Privat untuk mode tembus rintangan
        self.__skill_start_score = 0 #menandai kapan skill diaktifkan
        self.__frame_counter = 0 # menghitung 5 detik (sekitar 150 frame) 
        self.images = [
    ...
    def reset_char(self):
        ...
        self.__skill_start_score = 0
        self.__frame_counter = 0

    def cast_skill(self, obstacle_objects = None, ghost_object = None):
        "Ghost Mode, Dapat menembus rintangan selama 5 detik"
        if self.__idObject != "002": #memastikan hanya onre yang bisa menggunakan skill
            return
        ...
        if self.score % 10 == 0 and self.score > 0 and not self.__ghost_mode:
            ...
            self.skill = True
            self.__skill_start_score = self.score
        ...
        if self.__ghost_mode:
            ...
            if self.__frame_counter <= 0:
                self.__ghost_mode = False
                self.skill = False
                # mengembalikan transparansi karakter
                for i in range(len(self.images)):
...
class char3(Character):
    ...
    def __init__(self):
        ...
        self.__idObject = "003"
        self.skill = False
        self.__double_score = False # Privat untuk mode double score
        self.__skill_start_score = 0 #menyimpan score saat skill diaktifkan
        self.images = [
    ...
    def cast_skill(self, obstacle_objects = None, ghost_object = None):
        ...
        if self.__idObject != "003": #memastikan hanya yurei yang bisa menggunakan skill
            return
        ...
        for i in range(len(self.images)):
            self.images[i].set_alpha(255)
            self.image = self.images[self.current_img]
            print("Yurei Double Score Mode Deactiv at score {self.score}")
