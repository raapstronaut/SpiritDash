...
def show_score(text, font, color, x, y):
    "Display Score menggunakan bloodlust"
    bloodlust_font = pygame.font.Font(fntGame, 30)
    img = font.render(text, True, color)
    screen.blit(img, (x, y))

def show_gameover(text, font, color, x, y):
    euro_font = pygame.font.Font(fntover, 30)
    text_surface = euro_font.render(text, True, color)
